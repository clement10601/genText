...
def genImageAndLabel(text):
    ...
    im_width = len(text) * im_height * ratio / (2 if isEnglish else 1)
    ...
    for dirPath, dirNames, fileNames in os.walk(fontPath):
        for index, f in enumerate(fileNames):
            ...
            # print font
            im = Image.new(
                'RGB', (int(im_width), im_height), color=(
                    b_r, b_g, b_b))

            draw = ImageDraw.Draw(im)
            draw.text((0, 0),
                      text,
                      font=ImageFont.truetype(font, size=int(imageHeight*0.6)),
                      fill=(f_r, f_g, f_b))
...
def sentenceGenImage(fileName, maxNumber):
    ...
    with open(fileName) as f:
        # remove white space and convert to utf-8
        #context = str( f.read().strip(), "utf-8" )
        context = f.read().strip()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--text", type=lambda s: str(s, 'utf8'),
